  ...
  */
  const executeCommandAsync = (cmd) => new Promise((resolve) => {
    ...
    proc.on('close', (code) => {
      ...
      if(code !== 0 && stackAnalize(executionStack)) {
        commandToRerun = cmd
      } else {
  ...
  async function reRunner(commandsArray) {
    ...
    const failedTests = await new Array(commandRunTriesCount).fill(commandRunTriesCount).reduce((resolver) => {
      return resolver.then((resolvedCommandsArray) => runCommandsArray(resolvedCommandsArray, [])
    ...
    */
    ...
    async function runCommandsArray(commands, failedCommands) {
      const asserter = setInterval(() => runCommand(commands, failedCommands), intervalPoll);

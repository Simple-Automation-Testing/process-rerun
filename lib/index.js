...
function buildScope(runOptions) {
  ...
  */
  const runPromise = (cmd) => new Promise((res) => {
    ...
    /**
     * @param {undefined|function} formCommanWithOption if cuntion cmd will go to this function as argument
     */
    ...
    proc.stdout.on('data', (data) => {(debugProcess) && console.log(data.toString('utf8')); fullStack += data.toString()})

...
const {sleep, getRunCommand, walkSync} = require('./helpers')
...
function buildScope(runOptions) {
  ...
  const {stackAnalize, specRerunCount, grepWord, currentSessionCount, debugProcess} = runOptions
  ...
  const runPromise = (cmd) => new Promise((res) => {
    ...
    (argv.log || debugProcess) && console.log(cmd)

...
describe('executor circle', () => {
  it('[p] circleExecutor longestProcessTime kill procs', async function () {
  ...
  it('[p] circleExecutor longestProcessTime ', async function () {
    const commands = [`node -e 'setTimeout(() => console.log("Success first"), 5000)'`];
    const result = await circleExecutor(
      {
        maxThreads: 4,
  ...
  it('[p] circleExecutor longestProcessTime  kill processResultAnalyzer return null notRetriable auto push', async function () {
    const commands = [
  ...
  it('[p] circleExecutor longestProcessTime  kill processResultAnalyzer return cmd notRetriable auto push', async function () {
    const commands = [`node -e 'setTimeout(() => console.log("Success first"), 5000)'`];
    const result = await circleExecutor(
      {
        maxThreads: 4,
  ...
  it('[p] circleExecutor processResultAnalyzer arguments', async function () {
    const commands = [
  ...
  it('[p] circleExecutor successExitCode', async function () {
